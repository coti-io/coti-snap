...
const getStatusIndicator = (status: SyncStatus) => {
  switch (status) {
    ...
    case 'skipped':
      return <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="9" fill="#e5e7eb"/><path d="M6 9h6" stroke="#8a8f98" strokeWidth="1.5" strokeLinecap="round"/></svg>;
    case 'error':
      return <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="9" fill="#e53935"/><path d="M6.5 6.5l5 5M11.5 6.5l-5 5" stroke="#fff" strokeWidth="1.5" strokeLinecap="round"/></svg>;
    default:
      return <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="8" stroke="#e5e7eb" strokeWidth="1.5" fill="none"/></svg>;
  }
};

export const SyncSnapModal: React.FC<SyncSnapModalProps> = React.memo(({ open, onClose }) => {
  ...
  useEffect(() => {
    if (open && importedTokens.length > 0) {
      setTokenStates(importedTokens.map(token => ({ token, status: 'pending' })));
    }
  }, [open, importedTokens]);

  const syncedCount = tokenStates.filter(t => t.status === 'success').length;
  const skippedCount = tokenStates.filter(t => t.status === 'skipped').length;
  ...
  const handleSync = useCallback(async () => {
    if (isSyncingRef.current || tokenStates.length === 0) return;
    ...
    for (let i = 0; i < tokenStates.length; i++) {
      ...
      setTokenStates(prev => prev.map((item, idx) =>
      ...
      try {
        ...
        setTokenStates(prev => prev.map((item, idx) =>
          idx === i ? { ...item, status: isAlreadyExists ? 'skipped' : 'error', message: isAlreadyExists ? 'Already in snap' : msg } : item
        ));
      }
    }
  ...
  return (
    <ModalBackdrop onClick={handleBackdropClick}>
      <AnimatedModalContainer onClick={e => e.stopPropagation()} onKeyDown={handleKeyDown} tabIndex={-1}>
        <ModalHeader>
          Sync to Snap
          <ModalClose onClick={handleClose} aria-label="Close modal" type="button">
            &times;
          </ModalClose>
        </ModalHeader>
        ...
        <ModalBody>
          {phase === 'ready' && (
            <Description>
              Sync your imported tokens and NFTs to the COTI Snap so they appear inside COTI Snap.
            </Description>
          )}
                ...
                {tokenStates.map((item, idx) => (
                  <TokenRow key={item.token.address + idx}>
                    <TokenInfo>
                      ...
                      <div>
                        <TokenName>
                          {item.token.name} <TokenType>{item.token.symbol}</TokenType>
                        </TokenName>
                ...
                <SummaryRow>
                  ...
                  {skippedCount > 0 && (
                    <SummaryStat color="#8a8f98">
                      <StatusDot color="#d1d5db" /> {skippedCount} already synced
                    </SummaryStat>
                  )}
                  {errorCount > 0 && (
        ...
        <ButtonArea>
          ...
          {phase === 'done' && (
            <SendButton onClick={handleClose} type="button" backgroundColor="#10b981">
              Done
            </SendButton>
          )}
          {tokenStates.length === 0 && (
            <SendButton onClick={handleClose} type="button" backgroundColor="#fff" textColor="#1E29F6">
              Close
            </SendButton>
          )}
        </ButtonArea>
